...
被提升的节点只会创建一次，render函数会随着组件的更新执行
...
1. 由于props中的onclick的事件处理函数很少被修改，vue3内部将其缓存起来，不做diff算法
...
这个根组件经过createBlock后，会有一个额外的属性，dynamicChildren，span的这个vnode加入了dynamicChildren中

此时进行比较的话，不需要检查每个vnode的变化，而是直接去找block，进行扁平数组的迭代

而且此时/*TEXT*/标志着，只需要检查文本内容，而不需要担心其他的props的变化
